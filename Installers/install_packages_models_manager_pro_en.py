...
MESSAGES.update({'user_site_mode': 'Administrator rights are not available and no virtual environment is active. pip installs will use --user and the script will not try to uninstall global packages.', 'global_mode': 'Standard pip installation will be used in the active Python environment.', 'skip_uninstall_user_mode': 'Skipping pip uninstall because the script is running in --user mode and removing global packages from Program Files without administrator rights is not safe.', 'cmake_msi_fallback': 'The MSI-based CMake installation did not finish cleanly. A portable ZIP build will be used as a fallback.', 'cmake_zip_downloading': 'Downloading the CMake ZIP from the official release source...', 'cmake_zip_downloaded': 'The CMake ZIP was saved to: {path}', 'cmake_portable_ready': 'The portable CMake build is ready at: {path}', 'cmake_user_path_persisted': 'CMake was also added to the Windows user PATH.', 'cmake_user_path_failed': 'Could not update the Windows user PATH: {error}', 'torch_reuse': 'A working torch/torchvision installation was already found ({version}). It will be reused as-is and no reinstall will be attempted.', 'torch_reuse_gpu': 'A working GPU-enabled torch/torchvision installation was already found ({version}) with torch.cuda.is_available()=True. Reinstallation will be skipped.', 'torch_existing_not_usable': 'An existing torch/torchvision installation was found, but it does not match the requested mode ({variant}). A compatible version will be installed.'})
...
class Installer:
    def __init__(self, dry_run: bool = False, force_gpu: bool = False, force_cpu: bool = False, skip_optional: bool = False) -> None:
        ...
        self.use_user_site = (not in_virtualenv()) and (not is_windows_admin())
    ...
    def pip(self, args: list[str], *, required: bool = True) -> CommandResult:
        effective = list(args)
        if effective and effective[0] == "install" and self.use_user_site and "--user" not in effective:
            effective.insert(1, "--user")
            effective.insert(2, "--no-warn-script-location")
        return self.run([self.python, "-m", "pip", *effective], required=required)

    def install(self, packages: Iterable[str], *, index_url: str | None = None, required: bool = True, ignore_installed: bool = False) -> CommandResult:
        cmd = ["install", "--upgrade"]
        if ignore_installed:
    ...
    def uninstall(self, packages: Iterable[str]) -> CommandResult:
        if self.use_user_site:
            print(msg("skip_uninstall_user_mode"))
            return CommandResult(True, 0, [self.python, "-m", "pip", "uninstall", "-y", *packages])
...
CMAKE_ZIP_NAME = "cmake-4.3.0-windows-x86_64.zip"
CMAKE_ZIP_URL = "https://github.com/Kitware/CMake/releases/download/v4.3.0/cmake-4.3.0-windows-x86_64.zip"
...
def add_cmake_to_current_path() -> str | None:
    candidates = [
        ...
        Path.home() / "AppData" / "Local" / "Programs" / f"CMake-{CMAKE_VERSION_TEXT}-portable" / f"cmake-{CMAKE_VERSION_TEXT}-windows-x86_64" / "bin",
        Path.home() / "AppData" / "Local" / "Programs" / f"CMake-{CMAKE_VERSION_TEXT}-portable" / "bin",
    ]
...
def persist_user_path(path_entry: str) -> None:
    if os.name != "nt":
        return
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0, winreg.KEY_READ | winreg.KEY_WRITE) as key:
            ...
            if path_entry not in parts:
                new_value = current + (os.pathsep if current else "") + path_entry
                winreg.SetValueEx(key, "PATH", 0, winreg.REG_EXPAND_SZ, new_value)
                print(msg("cmake_user_path_persisted"))
    except Exception as exc:
        print(msg("cmake_user_path_failed", error=str(exc)))
...
def resolve_cmake_zip_path(inst: Installer) -> Path:
    script_dir = Path(__file__).resolve().parent
    local_candidates = [
        script_dir / CMAKE_ZIP_NAME,
        Path.cwd() / CMAKE_ZIP_NAME,
        Path(tempfile.gettempdir()) / CMAKE_ZIP_NAME,
    ]
    for candidate in local_candidates:
        if candidate.exists():
            return candidate
    download_path = Path(tempfile.gettempdir()) / CMAKE_ZIP_NAME
    print(msg("cmake_zip_downloading"))
    if inst.dry_run:
        print(msg("cmake_zip_downloaded", path=str(download_path)))
        return download_path
    urllib.request.urlretrieve(CMAKE_ZIP_URL, download_path)
    print(msg("cmake_zip_downloaded", path=str(download_path)))
    return download_path


def install_cmake_zip_fallback(inst: Installer) -> None:
    zip_path = resolve_cmake_zip_path(inst)
    target_root = Path.home() / "AppData" / "Local" / "Programs" / f"CMake-{CMAKE_VERSION_TEXT}-portable"
    inner_root = target_root / f"cmake-{CMAKE_VERSION_TEXT}-windows-x86_64"
    if inst.dry_run:
        print(msg("cmake_portable_ready", path=str(inner_root)))
        return
    ...
    bin_path = inner_root / "bin"
    if bin_path.exists():
        os.environ["PATH"] = str(bin_path) + os.pathsep + os.environ.get("PATH", "")
        print(msg("cmake_path_added", path=str(bin_path)))
        persist_user_path(str(bin_path))
    print(msg("cmake_portable_ready", path=str(inner_root)))
...
def ensure_cmake(inst: Installer, explicit_path: str = "", skip_cmake: bool = False) -> None:
    ...
    msi_result = inst.run(cmd, required=False)
    added = add_cmake_to_current_path()
    version_after = get_cmake_version()
    if version_after:
        if added:
            persist_user_path(added)
        print(msg("cmake_ready", version=version_after))
        return

    print(msg("cmake_not_found_postinstall"))
    print(msg("cmake_msi_fallback"))
    try:
        install_cmake_zip_fallback(inst)
    except Exception as exc:
        inst.failures.append(str(exc))
    version_after = get_cmake_version()
    if version_after:
        ...
        inst.failures.append(msg("cmake_not_found_postinstall"))
...
def ensure_triton_windows(inst: Installer, gpu_mode: bool) -> None:
    ...
    inst.install(["triton-windows"], required=False, ignore_installed=inst.use_user_site)
...
def ensure_tensorrt(inst: Installer, gpu_mode: bool) -> None:
    ...
    inst.install(["tensorrt-cu12"], required=False, ignore_installed=inst.use_user_site)
    if inst.dry_run or module_available("tensorrt"):
        return

    print(msg("tensorrt_fallback", package="tensorrt"))
    inst.install(["tensorrt"], required=False, ignore_installed=inst.use_user_site)
...
def inspect_existing_torch() -> dict[str, object] | None:
    try:
        ...
        return {
            ...
            "torchvision_version": str(tv_version),
...
def install_pytorch(inst: Installer, variant: str, index_url: str | None) -> None:
    existing = inspect_existing_torch()
    if existing:
        version_text = f"{existing.get('torch_version')} / torchvision {existing.get('torchvision_version')}"
        if variant == "cpu":
            print(msg("torch_reuse", version=version_text))
            return
        if bool(existing.get("cuda_available")):
            print(msg("torch_reuse_gpu", version=version_text))
            return
        print(msg("torch_existing_not_usable", variant=variant))

    print(msg("installing_torch", variant=variant))
    ignore_installed = inst.use_user_site
    if variant == "cpu":
        if sys.platform == "darwin":
            inst.install(["torch", "torchvision"], required=True, ignore_installed=ignore_installed)
        else:
            inst.install(["torch", "torchvision"], index_url="https://download.pytorch.org/whl/cpu", required=True, ignore_installed=ignore_installed)
    else:
        inst.install(["torch", "torchvision"], index_url=index_url, required=True, ignore_installed=ignore_installed)


def install_runtime_packages(inst: Installer, gpu_mode: bool) -> None:
    if gpu_mode:
        print(msg("installing_ort_gpu"))
        inst.install(["onnxruntime-gpu[cuda,cudnn]"], required=True, ignore_installed=inst.use_user_site)
    else:
        print(msg("installing_ort_cpu"))
        inst.install(["onnxruntime"], required=True, ignore_installed=inst.use_user_site)
...
def main() -> int:
    ...
    if inst.use_user_site:
        print(msg("user_site_mode"))
    else:
        print(msg("global_mode"))
